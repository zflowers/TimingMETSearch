...
void Timing_ZZ_to_4l(std::string output_name =
			      "output_Timing_ZZ_to_4l.root"){
    ...
    vector<double> sigmaT; //resolution on SVs in microns
  ...
  treePlot->Draw("GenTree", "Generator Tree", true);
    ...
    Vertex SV_true(0.0,0.0,0.0,0.0); //nominal SV at 0
    Resolution test_Resolution(PUPPI_Detector); //setting up the Resolution "calculator"
    double LAB_Pt;
    double LAB_eta;
    double LAB_M;
    
    //uncertainty on the distance between PV and SV
  ...
  for(int m = 0; m < NsigmaT; m++){
    g_Log << LogInfo << "Generating ZZ BKG events for ";
    ...
    g_Log << LogInfo << "Processing " << Ngen*NsigmaT << " Events" << LogEnd;
    histPlot->Draw(true);
  ...
  g_Log << LogInfo << "Time to Process " << Ngen*NsigmaT << " Events: " << (Long64_t(gSystem->Now())-end)/1000.0 << " seconds" << LogEnd;
}
